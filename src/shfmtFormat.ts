...
export class ShfmtFormatter {
    ...
    public checkEnv() {
        ...
        if (!configBinPath && !this.isExecutedFmtCommand()) {
            vscode.window.showErrorMessage('shellformat.path not config and shfmt not found under path! please download or run go get -u github.com/mvdan/sh/cmd/shfmt to install');
        }
    }
